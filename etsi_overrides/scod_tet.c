...
static void acelp_scod_rebind_ptrs(void)
{
	new_speech = old_speech + L_total - L_frame;
	speech = new_speech - L_next;
...
void acelp_scod_state_get(acelp_scod_state_t *state)
{
	...
	for (i = 0; i < L_total; i++) {
		state->old_speech[i] = old_speech[i];
	}
	for (i = 0; i < (L_frame + pit_max); i++) {
		state->old_wsp[i] = old_wsp[i];
	}
	for (i = 0; i < (L_frame + pit_max + L_inter); i++) {
		state->old_exc[i] = old_exc[i];
	}
	for (i = 0; i < (L_subfr + pp1); i++) {
		state->ai_zero[i] = ai_zero[i];
	}
	for (i = 0; i < p; i++) {
		state->f_gamma1[i] = F_gamma1[i];
		state->f_gamma2[i] = F_gamma2[i];
		state->f_gamma3[i] = F_gamma3[i];
		state->f_gamma4[i] = F_gamma4[i];
		state->lspold[i] = lspold[i];
		state->lspnew[i] = lspnew[i];
		state->lspnew_q[i] = lspnew_q[i];
		state->lspold_q[i] = lspold_q[i];
		state->mem_syn[i] = mem_syn[i];
		state->mem_w0[i] = mem_w0[i];
		state->mem_w[i] = mem_w[i];
	}
	for (i = 0; i < dim_rr; i++) {
		for (j = 0; j < dim_rr; j++) {
			state->rr[i][j] = rr[i][j];
	...
	state->last_ener_cod = last_ener_cod;
	state->last_ener_pit = last_ener_pit;
}
...
void acelp_scod_state_set(const acelp_scod_state_t *state)
{
	...
	for (i = 0; i < L_total; i++) {
		old_speech[i] = state->old_speech[i];
	}
	for (i = 0; i < (L_frame + pit_max); i++) {
		old_wsp[i] = state->old_wsp[i];
	}
	for (i = 0; i < (L_frame + pit_max + L_inter); i++) {
		old_exc[i] = state->old_exc[i];
	}
	for (i = 0; i < (L_subfr + pp1); i++) {
		ai_zero[i] = state->ai_zero[i];
	}
	for (i = 0; i < p; i++) {
		F_gamma1[i] = state->f_gamma1[i];
		F_gamma2[i] = state->f_gamma2[i];
		F_gamma3[i] = state->f_gamma3[i];
		F_gamma4[i] = state->f_gamma4[i];
		lspold[i] = state->lspold[i];
		lspnew[i] = state->lspnew[i];
		lspnew_q[i] = state->lspnew_q[i];
		lspold_q[i] = state->lspold_q[i];
		mem_syn[i] = state->mem_syn[i];
		mem_w0[i] = state->mem_w0[i];
		mem_w[i] = state->mem_w[i];
	}
	for (i = 0; i < dim_rr; i++) {
		for (j = 0; j < dim_rr; j++) {
			rr[i][j] = state->rr[i][j];
	...
	last_ener_cod = state->last_ener_cod;
	last_ener_pit = state->last_ener_pit;
	acelp_scod_rebind_ptrs();
}
